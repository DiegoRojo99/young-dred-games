const MOVE_TYPES = ['rock', 'paper', 'scissors', 'lizard', 'spock'];
...
function playGame(roundNumber) {
  ...
  const resultText = getResultText(pointsPlayerOne,pointsPlayerTwo);
  ...
  $playerOneMove.find('.icon').attr('src', `./img/rps/${playerOneMove.type}.png`);
  $playerOneMove.find('.name').text(playerOneMove.type);
  $playerOneMove.find('.attack').text(playerOneMove.value + ' Attack Points');
  $playerTwoMove.find('.icon').attr('src', `./img/rps/${playerTwoMove.type}.png`);
  ...
  $('#round .result .text').text(resultText);
  ...
  $('#round .result').delay(5000).fadeIn(0, () => {
    $ROUND_SOUND.currentTime = 0;
    $ROUND_SOUND.play();
  });
...
function resetMoves(player) {
  ...
  $('#move-one-type .icon').attr('src', './img/rps/rock.png');
  $('#move-one-type .name').text('rock');
  $('#move-two-type').data('type', 'paper');
  $('#move-two-type .icon').attr('src', './img/rps/paper.png');
  $('#move-two-type .name').text('paper');
  $('#move-three-type').data('type', 'scissors');
  $('#move-three-type .icon').attr('src', './img/rps/scissors.png');
...
function changeMoveType($typeElement, direction) {
  ...
  let newType;
  if (direction === 'left') {
    newType = MOVE_TYPES[(MOVE_TYPES.indexOf(currentType) + 4) % 5];
  } else {
    newType = MOVE_TYPES[(MOVE_TYPES.indexOf(currentType) + 1) % 5];
  }
  const newTypeSrc = `./img/rps/${newType}.png`;
...
function getResultText(points1,points2) {
